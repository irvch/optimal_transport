...
function l = L(x, y, Tx, Hx, Hy, lam, weights)
    l = C(x, Tx, weights) + lam*(KL(Tx, y, Tx, Hx, Hy, weights));
end
...
function gradF = KL_grad(Tx1, y, Tx2, Hx, Hy, weights)
    [n, d] = size(Tx1);
    m = length(y);
...
function gradL = L_grad(x, y, Tx, Hx, Hy, lam, weights)
    gradL = C_grad(x, Tx, weights) + lam * KL_grad(Tx, y, Tx, Hx, Hy, weights);
end
...
function [T_hist, L1_hist, L2_hist, L_hist, eta_hist, H_hist, iter, min_index] = grad_descent(x, y, eta, lam)
    ...
    Hy = bandwidth(y, length(x), true);       % BANDWIDTH FOR Y
    Hz = bandwidth(z, length(x), true);       % BANDWIDTH FOR ALL POINTS
    ...
    L2_hist = KL(Tx, y, Tx, Hz, Hy, weights);
    ...
    while criteria > 0
        ...
        L2_hist(iter,:) = KL(Tx, y, Tx, Hz, Hy, weights);
