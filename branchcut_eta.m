...
N_half = 10; % Number of points per semi-circle
...
addition = mvnrnd([2, -2], [0.01, 0; 0, 0.01], 3);
...
function [c, y, A_eq, A_ineq, b_eq, b_ineq, M, N] = constraints(source, target, lambda, p, q)
    % COST MATRIX
    %C = computeCostMatrix(source, target);
...
function [T, fval] = OT(X, Y, p, q)
    % COST MATRIX MADE WITH FROBENIUS NORM
    C = pdist2(X, Y);
    [N, M] = size(C);

    % OBJECTIVE FUNCTION: FLATTEN COST MATRIX
    C_flat = reshape(C.',1,[]).';
    f = C_flat;
    ...
    % LOWER BOUNDS FOR NON-NEGATIVE CONSTRAINTS
    lb = zeros(N * M, 1);

    % USE MATLAB LINPROG SOLVER
    options = optimoptions('linprog', 'Algorithm', 'dual-simplex');
    [Tvec, fval] = linprog(f, [], [], Aeq, beq, lb, [], options);

    % RESHAPE SOLN BACK INTO MATRIX FORM
    T = reshape(Tvec, [M, N]);
end
...
function [p_or_q_original, seen] = threshold(p_or_q, p_or_q_next, epsilon, letter, seen)
    % UNIFORM MASS
    nonzero_indices = find(p_or_q ~= 0);
    nonzero_count = numel(nonzero_indices);
    unif = 1 / nonzero_count;
    ...
    p_or_q_original = normalize(p_or_q);
end
...
function p_or_q_update = fix(p_or_q, p_or_q_next, epsilon, index)
    ...
    p_or_q_update = normalize(p_or_q);
end
...
function [T_best, cval_best, p_best, q_best, alpha_best, beta_best] = branch(source, target, p, q, mu, lambda, epsilon, iters)
    % CREATE CONSTRAINT MATRICES
    [c, y, Aeq, A_ineq, beq, b_ineq, M, N] = constraints(source, target, lambda, p, q);

    % INITIAL RUN
    [~, ~, p_new, q_new, ~, ~] = partialOT(c, y, A_ineq, b_ineq, Aeq, beq, p, q, mu, M, N, iters);
    ...
    [T, cval, p_new, q_new, alpha, beta] = partialOT(c, y, A_ineq, b_ineq, Aeq, beq, p, q, mu, M, N, iters);
    ...
    if ~isnan(p_index) || ~isnan(q_index)
        ...
        [T0, cval0, p0, q0, a0, b0] = partialOT(c, y, A_ineq, b_ineq, Aeq, beq, p, q, mu, M, N, iters);
        ...
        [T1, cval1, p1, q1, a1, b1] = partialOT(c, y, A_ineq, b_ineq, Aeq, beq, p, q, mu, M, N, iters);
    ...
    T(T <= 1e-6) = 0;
    p_best = p_new(p_new > 1e-6);
    q_best = q_new(q_new > 1e-6);
